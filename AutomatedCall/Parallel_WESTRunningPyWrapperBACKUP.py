...
class ModelInstances:
    ...
    def __init__(self, InstanceNames, modelPath, modelInternalName):
        ...
        self.modelInternalName = modelInternalName
    ...
    def storeModelOutput(self, ModelName, Parameterlist, Timelist, PathsArg):
        ...
        for fileout in [s for s in os.listdir(os.path.join(self.Paths, ModelName)) if '.out' in s]:
            shutil.move(os.path.join(self.Paths, ModelName, fileout), os.path.join(directory, 'Sim_' + str(counter)))
    ...
    def model_activateParallel(self, Parameterlist, Timelist, ModelName, ThreadingPool, ParamDatabase):
        ...
        PathsArg = {'Tornado_path': self.Paths + r"\\" + ModelName + "\Tornado.Main.xml",
                        'Experiment_path': self.Paths + r"\\" + ModelName + r"\\" + self.modelInternalName + r".xml"}  # Warning change the name of the model instance.Exp.xml.
